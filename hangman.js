...
var hangman = {
  ...
  startGame: function() {
    ...
    inquirer.prompt([{
      name: "play",
      type: "confirm",
      message: "Ready to play Legends of Hip-Hop Hangman?"
    }]).then(function(answer) {
      if (answer.play) {
        that.newGame()
      } 
      else {
        console.log("Don't be scared. Come back when you're ready to battle!")
      }
    })},
          ...
          if (found === 0) {
            ...
            console.log('Yes! You guessed right!')
              ...
              if (that.currentWord.didWeFindTheWord() === true) {
                console.log(that.currentWord.wordRender())
                console.log('Congratulations! You got skills!!! On to the next word!')
                that.startGame()
              } 
              else {
